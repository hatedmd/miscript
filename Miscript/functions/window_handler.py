...
class Window:
    ...
    def add_button(self, x, y, text="Click", on_click_callback=None, width=10, length=1, bg='lightgray', fg='black'):
        """Add button to window with onClick callback."""
        if self.window:
            try:
                button = tk.Button(
                    self.window, 
                    ...
                    bg=bg,  # Button background
                    fg=fg,  # Button text color
            ...
            try:
                ...
                if hex_color:
                    ...
                    for button in self.buttons.values():
                        button.config(bg='lightgray', fg='black')  # Keep buttons visible
                    return True
            except Exception as e:
...
def add_button_to_window(window_name, x, y, text, on_click_callback, width, length, bg='lightgray', fg='black'):
    """Add button to a stored window."""
    window = get_window(window_name)
    if window:
        return window.add_button(x, y, text, on_click_callback, width, length, bg, fg)
    return False
