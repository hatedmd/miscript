...
class Window:
    ...
    def __init__(self, width, height):
        ...
        self.buttons = {}
        self.variables = {}
        self.bg_color = 'white'  # Track background color
        ...
        if self.window:
            try:
                label = tk.Label(
                    ...
                    text=text, 
                    font=("Arial", 16), 
                    bg=self.bg_color,  # Use tracked bg color
                    fg='white'  # White text for visibility on dark backgrounds
                )
                label.place(x=x, y=y)
    ...
    def update_label(self, label_name, new_text):
        ...
        if label_name in self.labels and self.window:
            try:
                self.labels[label_name].config(text=new_text, bg=self.bg_color)
                self.window.update()
                return True
            except Exception as e:
    ...
    def add_button(self, x, y, text="Click", on_click_callback=None, width=10, length=1, colour='lightgray'):
        ...
        if self.window:
            try:
                ...
                fg_hex = 'black' if colour.lower() in ['white', 'yellow', 'lightgray', 'pink', 'cyan', 'silver'] else 'white'
    ...
    def set_background(self, color_type, color_value):
        ...
        if self.window:
            try:
                ...
                if hex_color:
                    self.bg_color = hex_color  # Track the color
                    self.window.configure(bg=hex_color)
                    
                    # Update ALL existing labels to match new background
