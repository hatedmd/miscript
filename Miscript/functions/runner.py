...
def _next_label_name():
    """Generate a unique label name."""
    _label_counter[0] += 1
    return f"label_{_label_counter[0]}"
...
def execute_function_body(function_body, variables, window_name):
    ...
    for stmt in function_body:
        if stmt['type'] == 'variable':
        ...
        elif stmt['type'] == 'window_method' and stmt['method'] == 'Text':
            # Inside a function, Text updates the dynamic label
            params = stmt['params']
            ...
            # Try to update an existing label at these coords first,
            # otherwise create a new one
            updated = update_label_in_window(window_name, 'dynamic_output', str(text))
            if not updated:
                add_text_to_window(window_name, str(text), int(x), int(y), label_name='dynamic_output')
    ...
    for stmt in statements:
        if stmt['type'] == 'print':
            ...
            if stmt['method'] == 'Text':
                ...
                # Use a unique label name per Text call so they don't overwrite each other.
                # Special case: if the text is a variable ref or empty placeholder,
                # mark it as the dynamic output label.
                if text_param.get('type') in ('variable_ref', 'entry_keyword') or str(text).strip() in ('', ' '):
                    label_name = 'dynamic_output'
                else:
                    label_name = _next_label_name()
                
                add_text_to_window(window_name, str(text), int(x), int(y), label_name=label_name)
                ...
                x_param = params.get('x', {'value': 0, 'type': 'number'})
                y_param = params.get('y', {'value': 0, 'type': 'number'})
                # Support explicit text: param, fall back to "Submit"
                text_param = params.get('text', {'value': 'Submit', 'type': 'string'})
                width_param = params.get('width', {'value': 10, 'type': 'number'})
                length_param = params.get('length', {'value': 1, 'type': 'number'})
                onclick_param = params.get('onClick', None)
                colour_param = params.get('colour', {'value': 'lightgray', 'type': 'named_color'})
                ...
                colour = resolve_param_value(colour_param, variables, window_name)
                ...
                add_button_to_window(window_name, int(x), int(y), str(text), callback, int(width), int(length), str(colour))
